...
public class ShaderReflection {
    private static Logger LOGGER = LoggerFactory.getLogger(ShaderReflection.class.getName());
        ...
        try (var stack = stackPush()) {
            ...
            for (var type : ResourceType.values()) {
                ...
                for (var reflect : reflectedResources) {
                    LOGGER.info(type.name() + " -> " + reflect.nameString() + " @ "
                            + spvc_compiler_get_decoration(compiler, reflect.id(), SpvDecorationBinding));
                }
            }
            // Destroy everything
